...
public class FemaleNames {
	...
	public static void main(String[] args) {
		...
		//Print in column format two names in each line
		
		for (int i=0; i<names.length; i+=2) {
			System.out.println(names[i]+", "+names[i+1]);
		}
		
		//print male names in single line by comma
		...
		System.out.println("Random names: "+names[r]);
		
		//print shortest name, less or equals to 4 characters, in the same line
		System.out.println("\nShortest name");
		
		for(String sh : names) {
			if(sh.length()<=4) {
				System.out.print(sh.toUpperCase() +", ");
			}
		}
		System.out.println("\nNames length sort");
		String namesUpto4 = "";
